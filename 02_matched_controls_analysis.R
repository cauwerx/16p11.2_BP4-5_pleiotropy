...
########################################################
# STEP 2: Define phenotypes to test
########################################################

# Load trait correlation data (generated by script/02_covariate_analysis/01_mediator_trait_correlation.R)
correlated_pheno <- as.data.frame(fread("/data/16p11.2_BP4-5_mediator_correlation.txt"))

# Identify traits with high correlation to BMI or EA
correlated_pheno_07 <- unique(correlated_pheno[which(abs(correlated_pheno$COR) > 0.7 & correlated_pheno$MEDIATOR != "height"), "PHENO"])
correlated_pheno_04 <- unique(correlated_pheno[which(abs(correlated_pheno$COR) > 0.4 & correlated_pheno$MEDIATOR != "height"), "PHENO"])
rm(correlated_pheno)


## Continuous traits ###################################

# Load BMI-corrected data; 
# Select significant vs non-significant associations
BMI_corrected_cont <- as.data.frame(fread("/data/correction_BMI/16p11.2_BP4-5_continuousPheWAS_BMIcorrected.txt"))
BMI_corrected_cont <- BMI_corrected_cont %>% 
                      group_by(PHENO) %>% 
                      slice(which.min(P))
BMI_corrected_cont_sig <- BMI_corrected_cont[which(BMI_corrected_cont$P <= 0.05/117), ]
BMI_corrected_cont_ns <- BMI_corrected_cont[which(BMI_corrected_cont$P > 0.05/117), ]


# Load EA-corrected data; 
# Select significant vs non-significant associations
EA_corrected_cont <- as.data.frame(fread("/data/correction_EA/16p11.2_BP4-5_continuousPheWAS_EAcorrected.txt"))
EA_corrected_cont <- EA_corrected_cont %>% 
                     group_by(PHENO) %>% 
                     slice(which.min(P))
EA_corrected_cont_sig <- EA_corrected_cont[which(EA_corrected_cont$P <= 0.05/117), ]
EA_corrected_cont_ns <- EA_corrected_cont[which(EA_corrected_cont$P > 0.05/117), ]

# Load height-corrected data; 
# Select significant vs non-significant associations
height_corrected_cont <- as.data.frame(fread("/data/correction_height/16p11.2_BP4-5_continuousPheWAS_HEIGHTcorrected.txt"))
height_corrected_cont <- height_corrected_cont %>% 
                          group_by(PHENO) %>% 
                          slice(which.min(P))
height_corrected_cont_sig <- height_corrected_cont[which(height_corrected_cont$P <= 0.05/117), ]
height_corrected_cont_ns <- height_corrected_cont[which(height_corrected_cont$P > 0.05/117), ]

# Compute the traits to be tested as the intersect, i.e., significant traits that never beccome unsignificant and are not too correlated with BMI or EA
pheno_sig <- unique(c(BMI_corrected_cont_sig$PHENO, EA_corrected_cont_sig$PHENO, height_corrected_cont_sig$PHENO))
pheno_ns <- unique(c(BMI_corrected_cont_ns$PHENO, EA_corrected_cont_ns$PHENO, height_corrected_cont_ns$PHENO))
pheno_select <- setdiff(pheno_sig, pheno_ns)
pheno_select <- setdiff(pheno_select, correlated_pheno_07)
...
# Load BMI-corrected data; 
# Select significant vs non-significant associations
BMI_corrected_disease <- as.data.frame(fread("/data/correction_BMI/16p11.2_BP4-5_binaryPheWAS_firth_BMIcorrected.txt"))
BMI_corrected_disease <- BMI_corrected_disease %>% 
                         group_by(PHENO) %>% 
                         slice(which.min(P))
BMI_corrected_disease_sig <- BMI_corrected_disease[which(BMI_corrected_disease$P <= 0.05/117), ]
BMI_corrected_disease_ns <- BMI_corrected_disease[which(BMI_corrected_disease$P > 0.05/117), ]


# Load EA-corrected data; 
# Select significant vs non-significant associations
EA_corrected_disease <- as.data.frame(fread("/data/correction_EA/16p11.2_BP4-5_binaryPheWAS_firth_EAcorrected.txt"))
EA_corrected_disease <- EA_corrected_disease %>% 
                        group_by(PHENO) %>% 
                        slice(which.min(P))
EA_corrected_disease_sig <- EA_corrected_disease[which(EA_corrected_disease$P <= 0.05/117), ]
EA_corrected_disease_ns <- EA_corrected_disease[which(EA_corrected_disease$P > 0.05/117), ]

# Load height-corrected data; 
# Select significant vs non-significant associations
height_corrected_disease <- as.data.frame(fread("/data/correction_EA/16p11.2_BP4-5_binaryPheWAS_firth_EAcorrected.txt"))
height_corrected_disease <- height_corrected_disease %>% 
                            group_by(PHENO) %>% 
                            slice(which.min(P))
height_corrected_disease_sig <- height_corrected_disease[which(height_corrected_disease$P <= 0.05/117), ]
height_corrected_disease_ns <- height_corrected_disease[which(height_corrected_disease$P > 0.05/117), ]

# Compute the traits to be tested as the intersect, i.e., significant traits that never beccome unsignificant and are not too correlated with BMI or EA
disease_sig <- unique(c(BMI_corrected_disease_sig$PHENO, EA_corrected_disease_sig$PHENO, height_corrected_disease_sig$PHENO))
disease_ns <- unique(c(BMI_corrected_disease_ns$PHENO, EA_corrected_disease_ns$PHENO, height_corrected_disease_ns$PHENO))
disease_select <- setdiff(disease_sig, disease_ns)
disease_select <- setdiff(disease_select, correlated_pheno_07)
...
t_test_results <- data.frame(PHENO = pheno_select, 
...
# Save results
fwrite(t_test_results, "/data/case_control/case_control_t_test.txt", col.names = T, row.names = F, quote = F, sep = "\t")


### Diseases #####################################

# Create an empty dataframe to store Fisher test results for diseases
fisher_results <- data.frame(PHENO = disease_select, 
...
# Save results
fwrite(fisher_results, "/data/case_control/case_control_fisher_test.txt", col.names = T, row.names = F, quote = F, sep = "\t")
...
palette <- data.frame(CAT = c("neurologic","musculoskeletal", "hepatic", "metabolic", "pulmonary", "hematologic", "reproductive", "renal"),
                      color = c(met.brewer("Signac")[11], met.brewer("Signac")[12], met.brewer("Signac")[5], met.brewer("Signac")[3], met.brewer("Signac")[10], met.brewer("Signac")[4], met.brewer("Signac")[2], met.brewer("Signac")[14]))
...
# Assemble plots & save
pdf("/plot/matched_controls.pdf", width = 12, height = 15)
showtext_auto() 
